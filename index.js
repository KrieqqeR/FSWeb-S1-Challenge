...
console.log("Profil :" + fenomenler[2].followers);
...
function fenomenEkle(fenomenler6,sırano,profileName,Takipci,post,platform) {
  const fenomenler6ödev = [...fenomenler6]
  const yeniNesne = {
    "number": sırano,
    "profile": profileName,
    "followers": Takipci,
  ...
  fenomenler6ödev.push(yeniNesne);

  return fenomenler6ödev;
}
...
function enFenomenler(fenomenler7parametre) {
  const yeniNesne7 = []
  for(let i = 0; i<fenomenler7parametre.length;i++){
    if(fenomenler7parametre[i].followers > 100000000){
       yeniNesne7.push(fenomenler7parametre[i].profile)
    }
  }
  return yeniNesne7;
}
...
function platformaGoreCokGonderiYapanFenomen(fenomenler9,platformName) {
  let enCokPost = 0;
  let enCokGonderiYapanFenomen = "";
  for(let i = 0; i<fenomenler9.length;i++){
    if(fenomenler9[i].platform === platformName && fenomenler9[i].post !== "NA" && fenomenler9[i].posts > enCokPost){
       enCokPost = fenomenler9[i].posts;
       enCokGonderiYapanFenomen = fenomenler9[i].profile;
    }
  }
